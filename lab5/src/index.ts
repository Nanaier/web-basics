...
const resolvers = {
  Query: {
    ...
    newsletter: async (_, { id }) =>
      await Newsletter.findById(id).populate("subscriber"),
  },
  Mutation: {
    createSubscriber: async (_, { input }) => await Subscriber.create(input),
    updateSubscriber: async (_, { id, input }) =>
      await Subscriber.findByIdAndUpdate(id, input, { new: true }),
    ...
    updateNewsletter: async (_, { id, input }) =>
      await Newsletter.findByIdAndUpdate(id, input, { new: true }),
...
mongoose
  .connect(
    "mongodb+srv://nanaier:02042004@maincluster.5nhdxbl.mongodb.net/newsletterSubscribers?retryWrites=true&w=majority"
  )
  .then(async () => {
